...
public enum RedisKey {
	 ...
	 * 不存在的 key 会被忽略.
	 * 
	 * 时间复杂度： O(N), N 为被删除的 key 的数量. 删除单个字符串类型的 key ,时间复杂度为O(1).
	 ...
	 * 序列化生成的值有以下几个特点：
	 * 
	 * 它带有 64 位的校验和,用于检测错误, RESTORE 在进行反序列化之前会先检查校验和. 值的编码格式和 RDB 文件保持一致. RDB
	 * 版本会被编码在序列化值当中,如果因为 Redis 的版本不同造成 RDB 格式不兼容,那么 Redis 会拒绝对这个值进行反序列化操作.
	 * 序列化的值不包括任何生存时间信息.
	 * 
	 * @param key
	 * @param value
	 ...
	 * another_key (以及它的生存时间)会被删除,然后旧的 key 会改名为 another_key ,因此,新的 another_key
	 * 的生存时间也和原本的 key 一样.
	 * 
	 * 使用 PERSIST 命令可以在不删除 key 的情况下,移除 key 的生存时间,让 key 重新成为一个『持久的』(persistent)
	 * key .
	 * 
	 * 更新生存时间
	 * 
	 ...
	 * 
	 * 命令的内部实现是这样的：它在当前实例对给定 key 执行 DUMP 命令 ,将它序列化,然后传送到目标实例,目标实例再使用 RESTORE
	 * 对数据进行反序列化,并将反序列化所得的数据添加到数据库中；当前实例就像目标实例的客户端那样,只要看到 RESTORE 命令返回 OK ,它就会调用
	 * DEL 删除自己数据库上的 key .
	 * 
	 * timeout 参数以毫秒为格式,指定当前实例和目标实例进行沟通的最大间隔时间.这说明操作并不一定要在 timeout
	 * 毫秒内完成,只是说数据传送的时间不能超过这个 timeout 数.
	 * 
	 * MIGRATE 命令需要在给定的时间规定内完成 IO 操作.如果在传送数据时发生 IO
	 * 错误,或者达到了超时时间,那么命令会停止执行,并返回一个特殊的错误： IOERR .
	 * 
	 * 当 IOERR 出现时,有以下两种可能：
	 * 
	 * key 可能存在于两个实例 key 可能只存在于当前实例 唯一不可能发生的情况就是丢失 key ,因此,如果一个客户端执行 MIGRATE
	 * 命令,并且不幸遇上 IOERR 错误,那么这个客户端唯一要做的就是检查自己数据库上的 key 是否已经被正确地删除.
	 * 
	 * 如果有其他错误发生,那么 MIGRATE 保证 key 只会出现在当前实例中.（当然,目标实例的给定数据库上可能有和 key 同名的键,不过这和
	 * MIGRATE 命令没有关系）.
	 * 
	 * 可选项：
	 * 
	 * COPY ：不移除源实例上的 key . REPLACE ：替换目标实例上已存在的 key .
	 * 
	 * @param key
	 * @param value
	 * @param ordinal
	 ...
	 * 
	 * 时间复杂度： O(1) 返回值： 改名成功时提示 OK ,失败时候返回一个错误
	 * 
	 * @param key
	 * @param value
	 * @return
	 ...
	 * 
	 * RESTORE 在执行反序列化之前会先对序列化值的 RDB 版本和数据校验和进行检查,如果 RDB 版本不相同或者数据不完整的话,那么
	 * RESTORE 会拒绝进行反序列化,并返回一个错误.
	 * 
	 * 如果键 key 已经存在, 并且给定了 REPLACE 选项, 那么使用反序列化得出的值来代替键 key 原有的值； 相反地, 如果键 key
	 * 已经存在, 但是没有给定 REPLACE 选项, 那么命令返回一个错误.
	 * 
	 * 
	 * @param key
	 * @param value
	 ...
	 * 
	 * SORT key 返回键值从小到大排序的结果. SORT key DESC 返回键值从大到小排序的结果. 假设 today_cost
	...
	/**
	 * SCAN 命令及其相关的 SSCAN 命令、 HSCAN 命令和 ZSCAN 命令都用于增量地迭代（incrementally
	 * iterate）一集元素（a collection of elements）：
	 * 
	 * SCAN 命令用于迭代当前数据库中的数据库键. SSCAN 命令用于迭代集合键中的元素. HSCAN 命令用于迭代哈希键中的键值对. ZSCAN
	 * 命令用于迭代有序集合中的元素（包括元素成员和元素分值）. 以上列出的四个命令都支持增量式迭代, 它们每次执行都只会返回少量元素,
	 * 所以这些命令可以用于生产环境, 而不会出现像 KEYS 命令、 SMEMBERS 命令带来的问题 —— 当 KEYS
	 * 命令被用于处理一个大的数据库时, 又或者 SMEMBERS 命令被用于处理一个大的集合键时, 它们可能会阻塞服务器达数秒之久.
	 * 
	 * 不过, 增量式迭代命令也不是没有缺点的： 举个例子, 使用 SMEMBERS 命令可以返回集合键当前包含的所有元素, 但是对于 SCAN
	 * 这类增量式迭代命令来说, 因为在对键进行增量式迭代的过程中, 键可能会被修改, 所以增量式迭代命令只能对被返回的元素提供有限的保证 （offer
	 * limited guarantees about the returned elements）.
	 * 
	 * 因为 SCAN 、 SSCAN 、 HSCAN 和 ZSCAN 四个命令的工作方式都非常相似, 所以这个文档会一并介绍这四个命令, 但是要记住：
	 * 
	 * SSCAN 命令、 HSCAN 命令和 ZSCAN 命令的第一个参数总是一个数据库键. 而 SCAN 命令则不需要在第一个参数提供任何数据库键
	 * —— 因为它迭代的是当前数据库中的所有数据库键. SCAN 命令的基本用法 SCAN 命令是一个基于游标的迭代器（cursor based
	 * iterator）： SCAN 命令每次被调用之后, 都会向用户返回一个新的游标, 用户在下次迭代时需要使用这个新游标作为 SCAN
	 ...
	 * 
	 * 它通常用在除错(debugging)或者了解为了节省空间而对 key 使用特殊编码的情况. 当将Redis用作缓存程序时,你也可以通过
	 * OBJECT 命令中的信息,决定 key 的驱逐策略(eviction policies).
	 * 
	 * OBJECT 命令有多个子命令：
	 * 
	 ...
	 * 则用于表示任何大小的有序集合.
	 * 
	 * 假如你做了什么让 Redis 没办法再使用节省空间的编码时(比如将一个只有 1 个元素的集合扩展为一个有 100
	 * 万个元素的集合),特殊编码类型(specially encoded types)会自动转换成通用类型(general type).
	 */
